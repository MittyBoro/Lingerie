...
Route::middleware(['admin.role:editor'])
	...
	Route::prefix('mail')->group(function () {
		...
		Route::get('product/{id?}', function ($id = null) {
			$invoice = App\Models\Product\ProductOrder::isPaid()->latest();
			$invoice = $id ? $invoice->findOrFail($id) : $invoice->first();
			return new App\Mail\ProductOrderPaid($invoice, null);
		});
		Route::get('feedback/{id?}', function ($id = null) {
			$invoice = App\Models\FeedbackOrder::latest()->first();
			$invoice = $id ? $invoice->findOrFail($id) : $invoice->first();
			return new App\Mail\FeedbackOrderSend($invoice, null);
		});
	});
